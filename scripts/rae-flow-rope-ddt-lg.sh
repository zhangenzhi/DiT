...
# DiT_RoPE_DDT: faithful two-stream wide-decoder DDT (~616M, FEWER params than uniform 677M)
# on lg 8 GPU. Same RAE latent + flow velocity + shift, PRECOMPUTED latents. Tests the
# decoupled (encoder semantics / wide per-token-conditioned decoder) architecture, not scaling.
module load gcc ompi
export MASTER_ADDR=$(head -n 1 $PBS_NODEFILE); export MASTER_PORT=29512
...
cd /work/c30636/DiT
: "${MAXSTEPS:=0}"; : "${GBS:=512}"   # 512 = 64/GPU on 8x H100 80GB; lr 1e-4 matches RAEv2 1024@2e-4 linearly
LATDIR=/work/c30636/dataset/rae_dinov3l_k7_latents/train
echo "RAE-ROPE-DDT-LG (two-stream, 12 cond tokens): MAXSTEPS=$MAXSTEPS GBS=$GBS latent=$LATDIR"
mpirun -np 2 --map-by ppr:1:node --bind-to none \
  ...
  ./train_rae_flow.py --latent-dir "$LATDIR" --arch dit_rope_ddt \
