...
# DiT_RoPE_WideHead (enc 28x1152 + 2x2048 head, ~840M) on lg 8 GPU. Same RAE latent
# + flow velocity + shift, PRECOMPUTED latents. Tests the wide-output-head lever.
module load gcc ompi
export MASTER_ADDR=$(head -n 1 $PBS_NODEFILE); export MASTER_PORT=29511
...
echo "RAE-ROPE-WIDE-LG (enc1152x28 + 2x2048 head): MAXSTEPS=$MAXSTEPS GBS=$GBS latent=$LATDIR"
mpirun -np 2 --map-by ppr:1:node --bind-to none \
  ...
  ./train_rae_flow.py --latent-dir "$LATDIR" --arch dit_rope_wide \
