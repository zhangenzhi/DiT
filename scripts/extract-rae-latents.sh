...
# Precompute RAE (DINOv3-K7) latents for ImageNet. vars: MAXCLS (0=all)
module load gcc ompi
export MASTER_ADDR=$(head -n 1 $PBS_NODEFILE); export MASTER_PORT=29505
...
echo "RAE extract: MAXCLS=$MAXCLS out=$OUT"
mpirun -np 1 --map-by ppr:1:node --bind-to none \
  ...
  ./extract_rae_latents.py --data-path /work/c30778/dataset/imagenet/train --out "$OUT" --batch-size 64 --max-classes "$MAXCLS"
