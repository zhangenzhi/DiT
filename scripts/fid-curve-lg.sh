...
# vars via qsub -v: CKDIR (subset symlink dir), MODEL, REPAFLAG(""/--repa), RESFILE(abs)
...
: "${REPAFLAG:=}"
echo "CURVE: MODEL=$MODEL CKDIR=$CKDIR REPAFLAG=$REPAFLAG RESFILE=$RESFILE"
mpirun -np 1 --map-by ppr:1:node --bind-to none \
  ...
  -x PATH -x CKDIR -x MODEL -x REPAFLAG -x RESFILE \
  ...
  --ckpt-dir "$CKDIR" --results-file "$RESFILE"
