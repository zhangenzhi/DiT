...
# DiT_RoPE + REPA (lambda from REPA_LAMBDA, default 0.5) on lg 8 GPU. Same RAE latent
# + flow velocity + shift, PRECOMPUTED latents. Tests whether the RMSNorm/QK-norm/RoPE
# backbone cures the ~135k REPA divergence seen on the vanilla DiT (exp 009/010).
module load gcc ompi
export MASTER_ADDR=$(head -n 1 $PBS_NODEFILE); export MASTER_PORT=29510
...
cd /work/c30636/DiT
: "${MAXSTEPS:=0}"; : "${GBS:=256}"; : "${REPA_LAMBDA:=0.5}"
LATDIR=/work/c30636/dataset/rae_dinov3l_k7_latents/train
echo "RAE-ROPE-REPA-LG: MAXSTEPS=$MAXSTEPS GBS=$GBS REPA_LAMBDA=$REPA_LAMBDA latent=$LATDIR"
mpirun -np 2 --map-by ppr:1:node --bind-to none \
  ...
  ./train_rae_flow.py --latent-dir "$LATDIR" --arch dit_rope --repa-lambda "$REPA_LAMBDA" \
