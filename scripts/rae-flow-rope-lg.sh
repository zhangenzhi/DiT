...
# Path A + RMSNorm/SwiGLU/RoPE backbone (DiT_RoPE) on lg 8 GPU: RAE (DINOv3-K7) latent
# + flow velocity + shift, PRECOMPUTED latents. vars: MAXSTEPS(0=full), GBS(256)
...
cd /work/c30636/DiT
: "${MAXSTEPS:=0}"; : "${GBS:=256}"; : "${RESUME:=}"; : "${LR_FINAL:=1e-5}"; : "${LR_DECAY_STEPS:=0}"
LATDIR=/work/c30636/dataset/rae_dinov3l_k7_latents/train
RESUME_ARG=""; [ -n "$RESUME" ] && RESUME_ARG="--resume $RESUME"
echo "RAE-ROPE-LG: MAXSTEPS=$MAXSTEPS GBS=$GBS RESUME=$RESUME LR_DECAY_STEPS=$LR_DECAY_STEPS LR_FINAL=$LR_FINAL"
mpirun -np 2 --map-by ppr:1:node --bind-to none \
  ...
  ./train_rae_flow.py --latent-dir "$LATDIR" --arch dit_rope $RESUME_ARG \
  --global-batch-size "$GBS" --learning-rate 1e-4 --lr-final "$LR_FINAL" --lr-decay-steps "$LR_DECAY_STEPS" \
  --ckpt-every 10000 --max-steps "$MAXSTEPS"
