...
# Sample from a Path-A RAE-flow checkpoint, decode via official RAE decoder, FID vs official ADM stats.
# vars: CKPT(abs .pt), NSAMP(10000), CFG(1.8), OUTFILE(abs)
module load gcc ompi
export MASTER_ADDR=$(head -n 1 $PBS_NODEFILE); export MASTER_PORT=29508
...
cd /work/c30636/DiT
: "${NSAMP:=10000}"; : "${CFG:=1.8}"; : "${ARCH:=dit}"
: "${OUTFILE:=/work/c30636/DiT/outputs/fid_rae_flow.txt}"
CKPT_S=$(basename "$CKPT" .pt)
echo "FID-RAE-FLOW: CKPT=$CKPT_S NSAMP=$NSAMP CFG=$CFG"
mpirun -np 1 --map-by ppr:1:node --bind-to none \
  ...
  ./sample_rae_flow.py --ckpt "$CKPT" --arch "$ARCH" --num-fid-samples "$NSAMP" --cfg-scale "$CFG" --per-proc-batch-size 32 --num-steps 100
NPZ="/work/c30636/DiT/outputs/rae_flow_samples/rae-flow-${CKPT_S}-cfg${CFG}-seed0.npz"
echo "npz: $NPZ"; [ -f "$NPZ" ] || { echo "ERROR no npz"; exit 1; }
FID=$(python fid_from_stats.py --samples-npz "$NPZ" | awk '/^FID-/{print $NF}')
echo "RESULT(rae-flow): $CKPT_S cfg=$CFG -> FID${NSAMP}=$FID"
echo "${CKPT_S}	cfg=${CFG}	FID${NSAMP}=${FID}" >> "$OUTFILE"
cat "$OUTFILE"
