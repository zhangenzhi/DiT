...
# Flow-matching (velocity) + REPA on DiT-XL/2, constant LR 1e-4, batch 512 (8 GPUs -> 64/GPU)
: "${NNODES:=2}"; : "${GBS:=512}"
echo "REPA-FLOW XL/2: nnodes=$NNODES batch=$GBS (flow/velocity, constant LR 1e-4)"
mpirun -np "$NNODES" \
    ...
    ./train_feat_repa_flow.py --model DiT-XL/2 \
    --features-path /work/c30636/dataset/dit_feat_repa/train \
    --dino-path /work/c30636/dataset/dit_feat_repa_dino/train \
    --global-batch-size "$GBS" --learning-rate 1e-4 --repa-lambda 0.5 --align-depth 8 \
    --ckpt-every 10000
