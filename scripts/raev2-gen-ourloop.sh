...
export MASTER_ADDR=$(head -n 1 $PBS_NODEFILE); export MASTER_PORT=29513
...
CFG=configs/stage2/sampling/imagenet-dinov3l-k7.yaml
OUT=/work/c30636/RAEv2/outputs/raev2_OURLOOP_n${NSAMP}.npz
...
mpirun -np 1 --map-by ppr:1:node --bind-to none \
  ...
  src/rae_gen_ourloop.py --num-steps 100 --config "$CFG" --num-samples "$NSAMP" --batch-size 32 --out "$OUT"
