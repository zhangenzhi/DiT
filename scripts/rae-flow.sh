...
# Path A: our DiT in the RAE (DINOv3-K7) latent + flow velocity + noise shift.
# vars: MAXSTEPS (0=full), GBS (256)
module load gcc ompi
export MASTER_ADDR=$(head -n 1 $PBS_NODEFILE); export MASTER_PORT=29504
...
cd /work/c30636/DiT
: "${MAXSTEPS:=0}"; : "${GBS:=256}"
echo "RAE-FLOW (Path A): MAXSTEPS=$MAXSTEPS GBS=$GBS"
mpirun -np 1 --map-by ppr:1:node --bind-to none \
  ...
  ./train_rae_flow.py --data-path /work/c30778/dataset/imagenet/train \
